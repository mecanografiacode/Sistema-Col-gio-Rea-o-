...
export const Marketing: React.FC<MarketingProps> = ({ currentUser }) => {
  ...
  const [activeTab, setActiveTab] = useState<'calendar' | 'kanban' | 'ai_generator' | 'ai_image' | 'plan' | 'metrics'>('calendar');
  ...
  const [imgPrompt, setImgPrompt] = useState('Post promocional para o Instagram do Colégio Reação com alunos participando de uma atividade prática de Robótica, cores vivas azul e vermelho, estilo moderno.');
  const [imgStyle, setImgStyle] = useState('Design Gráfico para Instagram');
  const [imgAspectRatio, setImgAspectRatio] = useState<'1:1' | '3:4' | '4:3' | '9:16' | '16:9'>('1:1');
  const [imgAttachments, setImgAttachments] = useState<{ name: string; data: string; mimeType: string }[]>([]);
  ...
  const [generatedImages, setGeneratedImages] = useState<{
  ...
  }[]>([
    {
      ...
      prompt: 'Alunos no laboratório de robótica e informática do Colégio Reação em atividade prática',
      description: 'Exemplo de arte para post do Colégio Reação.',
      createdAt: 'Exemplo',
      aspectRatio: '1:1'
    }
  ]);
      ...
      reader.onload = (e) => {
        ...
        if (dataStr) {
          setImgAttachments((prev) => [
            ...prev,
            { name: file.name, data: dataStr, mimeType: file.type }
          ]);
        }
      };
  ...
  const handleRemoveAttachment = (index: number) => {
    setImgAttachments((prev) => prev.filter((_, i) => i !== index));
  };
  ...
  const handleGenerateAiImage = async () => {
    ...
    try {
      ...
      if (data.success && data.imageUrl) {
        ...
        setImgError(data.error || 'Não foi possível gerar a imagem. Tente alterar o prompt ou anexos.');
      }
    } catch (err: any) {
      console.error('Error in handleGenerateAiImage:', err);
      setImgError('Erro de conexão com o servidor ao gerar imagem.');
    } finally {
      setImgLoading(false);
    }
  };

  const handleDownloadImage = (dataUrl: string, promptText: string) => {
    const link = document.createElement('a');
    link.href = dataUrl;
    link.download = `arte-colegio-reacao-${Date.now()}.png`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };

  const handleConvertImageToPost = (imgItem: { url: string; prompt: string }) => {
    setTitle(`Post com Arte IA: ${imgItem.prompt.slice(0, 40)}...`);
    setContentType('post_estatico');
    setCategory('Captação de Alunos');
    setAssetLink(imgItem.url);
    setNotes(`Arte gerada com IA Gemini a partir do prompt: "${imgItem.prompt}"`);
    setCaption(`✨ Arte oficial gerada com IA para o Colégio Reação!\n\n#ColegioReacao #EducacaoDF #RecantoDasEmas`);
    setScheduledDate(new Date().toISOString().split('T')[0]);
    setStatus('ideia');
    setIsNewModalOpen(true);
  };
    ...
    <div className="space-y-6 pb-20 lg:pb-8 font-['Plus_Jakarta_Sans',sans-serif]">
      ...
      <div className="bg-white p-1.5 rounded-2xl border border-slate-200 flex items-center gap-1.5 text-xs font-bold shadow-2xs overflow-x-auto scrollbar-none whitespace-nowrap">
        ...
        <button
          onClick={() => setActiveTab('ai_image')}
          className={`px-3.5 sm:px-4 py-2 rounded-xl transition-colors flex items-center space-x-2 shrink-0 ${
            activeTab === 'ai_image' ? 'bg-indigo-600 text-white shadow-xs' : 'text-indigo-700 bg-indigo-50 hover:bg-indigo-100'
          }`}
        >
          <ImageIcon className="w-4 h-4 text-indigo-400" />
          <span>Gerador de Imagens & Artes (IA)</span>
        </button>
      ...
      {/* --- TAB: GERADOR DE IMAGENS COM IA (GEMINI IMAGE) --- */}
      {activeTab === 'ai_image' && (
        <div className="space-y-6">
          <div className="bg-gradient-to-r from-indigo-900 via-purple-900 to-slate-900 p-6 rounded-2xl text-white shadow-md relative overflow-hidden">
            <div className="absolute top-0 right-0 p-8 opacity-10 pointer-events-none">
              <Wand2 className="w-48 h-48" />
            </div>
            <div className="relative z-10 space-y-2 max-w-2xl">
              <div className="inline-flex items-center space-x-1.5 px-3 py-1 bg-white/10 backdrop-blur-md rounded-full text-xs font-semibold text-indigo-200 border border-white/10">
                <Sparkles className="w-3.5 h-3.5 text-indigo-300" />
                <span>Gerador e Editor Visual de Imagens IA</span>
              </div>
              <h3 className="text-xl sm:text-2xl font-bold font-serif-editorial">
                Crie Artes e Edite Imagens com Prompts & Anexos
              </h3>
              <p className="text-xs sm:text-sm text-indigo-100/90 leading-relaxed">
                Descreva a imagem desejada para o Instagram ou envie imagens/fotos de referência (anexos) para a IA adaptar, combinar e gerar artes prontas para as redes sociais do Colégio Reação.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white p-5 sm:p-6 rounded-2xl border border-slate-200 shadow-xs space-y-5">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-700 flex items-center justify-between">
                <span>Prompt / Instruções da Imagem <span className="text-red-500">*</span></span>
                <span className="text-[11px] text-slate-400 font-normal">Descreva os detalhes, cenário e mensagem</span>
              </label>
              <textarea
                rows={3}
                value={imgPrompt}
                onChange={(e) => setImgPrompt(e.target.value)}
                placeholder="Ex: Crie um post para o Instagram do Colégio Reação celebrando as Olimpíadas de Robótica, com alunos felizes segurando troféus, cores azul e vermelho, estilo gráfico moderno e limpo."
                className="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl text-xs sm:text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-600 shadow-2xs resize-y"
              />

              {/* Quick Prompt Suggestions */}
              <div className="flex flex-wrap items-center gap-1.5 pt-1">
                <span className="text-[11px] font-semibold text-slate-500 mr-1">Sugestões de prompts:</span>
                <button
                  type="button"
                  onClick={() => setImgPrompt('Arte promocional para as Olimpíadas de Robótica do Colégio Reação com estudantes felizes e robôs, cores azul e vermelho.')}
                  className="px-2.5 py-1 bg-slate-100 hover:bg-indigo-50 hover:text-indigo-700 text-slate-600 text-[11px] font-medium rounded-lg border border-slate-200 transition-colors cursor-pointer"
                >
                  🤖 Robótica & Maker
                </button>
                <button
                  type="button"
                  onClick={() => setImgPrompt('Post do Instagram para Matrículas Abertas 2027 do Colégio Reação, com ambiente acolhedor, biblioteca moderna e logotipo institucional.')}
                  className="px-2.5 py-1 bg-slate-100 hover:bg-indigo-50 hover:text-indigo-700 text-slate-600 text-[11px] font-medium rounded-lg border border-slate-200 transition-colors cursor-pointer"
                >
                  🎓 Matrículas Abertas
                </button>
                <button
                  type="button"
                  onClick={() => setImgPrompt('Arte comemorativa do Dia dos Professores com mensagens de carinho, maçã, livros e paleta de cores acolhedoras do Colégio Reação.')}
                  className="px-2.5 py-1 bg-slate-100 hover:bg-indigo-50 hover:text-indigo-700 text-slate-600 text-[11px] font-medium rounded-lg border border-slate-200 transition-colors cursor-pointer"
                >
                  🍎 Dia dos Professores
                </button>
                <button
                  type="button"
                  onClick={() => setImgPrompt('Capa promocional de Feira de Ciências e Tecnologia com experimentos científicos infantis coloridos e iluminação moderna.')}
                  className="px-2.5 py-1 bg-slate-100 hover:bg-indigo-50 hover:text-indigo-700 text-slate-600 text-[11px] font-medium rounded-lg border border-slate-200 transition-colors cursor-pointer"
                >
                  🔬 Feira de Ciências
                </button>
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-700 flex items-center justify-between">
                <span className="flex items-center gap-1.5">
                  ...
                  <span>Anexos & Imagens de Referência (Opcional)</span>
                </span>
                <span className="text-[11px] text-slate-500 font-normal">Fotos da escola, logos ou referências visuais</span>
              </label>
              ...
              <div
                ...
                className="border-2 border-dashed border-indigo-200 hover:border-indigo-400 bg-indigo-50/40 hover:bg-indigo-50 p-4 sm:p-5 rounded-2xl text-center transition-colors cursor-pointer"
              >
                <input
                  type="file"
                  id="marketing-img-upload"
                  accept="image/*"
                  multiple
                  onChange={(e) => handleFileUpload(e.target.files)}
                  className="hidden"
                />
                <label htmlFor="marketing-img-upload" className="cursor-pointer space-y-2 block">
                  <div className="w-10 h-10 bg-indigo-100 text-indigo-600 rounded-xl flex items-center justify-center mx-auto">
                    <Upload className="w-5 h-5" />
                  </div>
                  <p className="text-xs font-bold text-slate-700">
                    Clique aqui ou arraste imagens / fotos de referência
                  </p>
                  <p className="text-[11px] text-slate-500">
                    Envie logos, fotos da fachada, fotos de alunos com LGPD ou artes inspiradoras (PNG, JPG, WEBP)
                  </p>
                </label>
              </div>
              ...
              {imgAttachments.length > 0 && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
                  {imgAttachments.map((att, idx) => (
                    <div key={idx} className="relative group bg-slate-50 rounded-xl border border-slate-200 p-2 flex flex-col items-center">
                      <div className="w-full h-24 rounded-lg overflow-hidden bg-slate-200 mb-1.5 relative">
                        <img src={att.data} alt={att.name} className="w-full h-full object-cover" />
                      </div>
                      <span className="text-[10px] font-bold text-slate-700 truncate w-full text-center px-1">
                        {att.name}
                      </span>
                      <button
                        type="button"
            ...
            {/* Options Grid: Style & Aspect Ratio */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-700">Estilo Visual da Imagem</label>
                <select
                  value={imgStyle}
                  onChange={(e) => setImgStyle(e.target.value)}
                  className="w-full px-3 py-2 border border-slate-300 rounded-xl text-xs text-slate-900 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-600"
                >
                  <option value="Design Gráfico para Instagram">Design Gráfico / Instagram Post (Oficial)</option>
                  <option value="Fotografia Realista Educacional">Fotografia Realista do Colégio</option>
                  <option value="Ilustração 3D Educacional">Ilustração 3D Digital Divertida</option>
                  <option value="Infográfico & Arte Vetorial">Infográfico / Arte Vetorial Minimalista</option>
                  <option value="Arte Conceitual Maker / Tecnologia">Arte Conceitual Tecnológica / Maker</option>
                </select>
              </div>

              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-700">Proporção (Aspect Ratio)</label>
                <div className="grid grid-cols-4 gap-1.5">
                  <button
                    type="button"
                    onClick={() => setImgAspectRatio('1:1')}
                  ...
                  <button
                    type="button"
                    onClick={() => setImgAspectRatio('9:16')}
                    className={`py-2 px-1 rounded-xl text-[11px] font-bold border text-center transition-all cursor-pointer ${
                      imgAspectRatio === '9:16'
                        ? 'bg-indigo-600 text-white border-indigo-600 shadow-2xs'
                        : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'
                  ...
                  <button
                    type="button"
                    onClick={() => setImgAspectRatio('16:9')}
                    className={`py-2 px-1 rounded-xl text-[11px] font-bold border text-center transition-all cursor-pointer ${
                      imgAspectRatio === '16:9'
                        ? 'bg-indigo-600 text-white border-indigo-600 shadow-2xs'
                        : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'
            ...
            <button
              ...
              className="w-full py-3.5 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 hover:from-indigo-700 hover:to-pink-700 text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow-md transition-all flex items-center justify-center space-x-2 disabled:opacity-50 cursor-pointer"
            >
              {imgLoading ? (
                <>
                  <RefreshCw className="w-4 h-4 animate-spin text-white" />
                  <span>A IA Gemini está gerando/editando sua imagem com os anexos...</span>
                </>
              ) : (
                <>
                  <Wand2 className="w-4 h-4 text-indigo-200" />
                  <span>Gerar Imagem / Arte Visual com IA</span>
                </>
              )}
            </button>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                ...
                <span>Galeria de Artes Geradas ({generatedImages.length})</span>
              </h3>
              <span className="text-xs text-slate-500">
                Artes prontas para agendar ou baixar
              </span>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {generatedImages.map((img) => (
                <div key={img.id} className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden flex flex-col hover:shadow-md transition-shadow">
                  <div className="relative bg-slate-900 group">
                    ...
                    <div className="absolute top-2 right-2 flex items-center gap-1">
                      <button
                        ...
                        className="p-2 bg-black/60 hover:bg-black/80 text-white rounded-xl backdrop-blur-md transition-colors cursor-pointer"
                        title="Ver em tamanho grande"
                      >
                        <Maximize2 className="w-4 h-4" />
                      </button>
                    </div>
                    <span className="absolute bottom-2 left-2 px-2.5 py-1 bg-black/60 backdrop-blur-md text-white text-[10px] font-bold rounded-lg border border-white/20">
                  ...
                  <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
                    <div className="space-y-1.5">
                      ...
                      <span className="text-[10px] text-slate-400 block pt-1">
                        Gerado às {img.createdAt}
                      </span>
                    </div>

                    <div className="pt-2 border-t border-slate-100 flex items-center gap-2">
                      <button
                        onClick={() => handleConvertImageToPost(img)}
                        className="flex-1 py-2 px-3 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                      >
                        <Plus className="w-3.5 h-3.5" />
      ...
      {/* --- MODAL PARA EXPANDIR E VER A IMAGEM GERADA --- */}
      {previewImageModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="relative max-w-4xl w-full max-h-[90vh] flex flex-col items-center">
            <button
              onClick={() => setPreviewImageModal(null)}
              className="absolute -top-10 right-0 p-2 text-white hover:text-slate-300 transition-colors cursor-pointer"
            >
              <X className="w-6 h-6" />
            </button>
            <img
            ...
            <div className="mt-4 flex items-center gap-3">
              <button
                onClick={() => handleDownloadImage(previewImageModal, 'arte-colegio-reacao')}
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-md flex items-center gap-2 cursor-pointer"
              >
                <Download className="w-4 h-4" />
